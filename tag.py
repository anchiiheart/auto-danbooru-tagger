...
def patch_IPTCInfo(*args, **kwargs):
    ...
    with HiddenPrints():
        return orignal_IPTCInfo(*args, **kwargs)
...
def win_addInfo(F,TagList):
    if not patched:
        orignal_IPTCInfo = iptcinfo3.IPTCInfo
...
def tag_eagle(root, TagList):
    with open(root + '\\' + "metadata.json", "r", encoding="utf8") as jsonFile:
        data = json.load(jsonFile)
        data["tags"] = TagList

    with open(root + '\\' + "metadata.json", "w", encoding="utf8") as jsonFile:
