...
class deepdanbooruModel():
    ...
    def load_model(self):
        print('loading model...')
        try:
            model = tf.keras.models.load_model("./deepdanbooru-v3-20211112-sgd-e28/model-resnet_custom_v3.h5", compile=False)
        except:
            print('Model not in folder. Download it from https://github.com/KichangKim/DeepDanbooru')
            sys.exit()
        with open("./deepdanbooru-v3-20211112-sgd-e28/tags.txt", 'r') as tags_stream:
    ...
    def classify_image(self, image_path):
        try:
            if (image_path[len(image_path)-4:len(image_path)] == '.psd'):
                psd = PSDImage.open(image_path)
                psd.composite().save('./temporary.png')
                image = np.array(PIL.Image.open('./temporary.png').convert(
                    'RGB').resize((512, 512))) / 255.0
            else:
